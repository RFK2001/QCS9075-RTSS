...
int main(int argc, char *argv[]) {
    ...
    while ((i < argc) && ((i+1) < argc))
    {
        if (strcmp(argv[i], "-sr") == 0) {
            ...
            if (sound_trigger_info.num_context > MAX_ACD_NUMBER_OF_CONTEXT) {
                printf("Invalid number_context, max allowed is %d\n", MAX_ACD_NUMBER_OF_CONTEXT);
                status = -EINVAL;
                goto error;
            }
            params++;
    ...
    if (!rec_config_payload) {
        printf("Memory allocation for rec_config_payload failed !\n");
        goto error;
    }
    ...
    printf("session params %p, %p, %d\n", &sound_trigger_info, rc_config, ses_handle);
