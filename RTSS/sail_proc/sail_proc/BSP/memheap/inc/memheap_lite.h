...
===========================================================================*/
/*
Copyright (c) 1997-2018 QUALCOMM Technologies Incorporated.
All rights reserved.
...
#define MEMHEAP_SUCCESS 0
#define MEMHEAP_FAILURE 1
...
typedef struct mem_block_header_struct {
  ...
  char          section_index:4;
  char          start_flag:1;
  char          free_flag:1;  /**< Flag to indicate if this memory block
                                   is free. */
  char          next_freeBlk_offset_sign:1; /* its set for free memory block only 
                                           used for traversing next free block  
                                           in the free block list in BINs
                                           TRUE- negative offset ; FALSE-positive offset */
  char          last_flag:1;/**< Flag to indicate if this is the last block
                                   in the allocated section. */
  uint32        forw_offset; /**< Forward offset. The value of the offset
                                   includes the size of the header and the
                                   allocated block. */
  uint32        prev_block_forw_offset; /*it will be set for used block
                                          and used for defragmentation in mem_free*/
  union{
    uint32      next_freeBlk_forw_offset; /*it will be set for "free memory block"
                                          for pointing to next free block in the free block list 
                                          in BINs  */
    uint32     caller_ptr; /*debug info only for "used blocks" stores return address of allocation API */
  };

} mem_block_header_type;

typedef struct _BIN{
  mem_block_header_type *firstNode;
  mem_block_header_type *lastNode;
  uint16 binToUseForMalloc;
  uint32 totalNodes;
}binStruct;

#define MEM_HEAP_SECTIONS_MAX     (4)
...
/**
  Returns the block size of the pointer

  @param[in] ptr        Pointer to the block previously allocated.
 */
...
/**
  ...
  @param[in] heap_ptr    Pointer to the heap from which to allocate.
  @param[in] mem_magic_number    Magic number structure to confirm correct magic number
  @param[in] ptr         Original pointer that needs to be reallocated.
  @param[in] heap_ptr    New size.
 ...
 */
/*lint -sem(mem_realloc,1p,2n>=0&&(@p==0||@P==2n)) */
void* mem_realloc(
  mem_heap_type *heap_ptr,
  mem_magic_number_struct       *mem_magic_number,
  void          *ptr,
...
typedef struct mem_heap_totals_struct {
  uint32 total_physical_bytes; /**< Total physical bytes in use. */
  uint32 current_block_count;  /**< Current block count in use. */
  uint32 used_blocks;          /**< Current blocks in use. */
  uint32 header_bytes;         /**< Amount of header bytes in use. */
  uint32 used_bytes;           /**< Amount of bytes in use. */
  uint32 free_bytes;           /**< Amount of free bytes. */
  uint32 wasted_bytes;         /**< Amount of extra unallocated 
                                           bytes in use that cannot be 
                                           allocated currently. */
  uint32 largest_free_block;   /**< Size of the largest free block. */
  uint32 max_logical_used;     /**< Maximum logical memory amount 
                                           used. */
  uint32 max_logical_request;  /**< Maximum logical amount requested. */
