...
#include "secboot_metadata_parser.h"
...
typedef enum secboot_error_t
{
  ...
  E_SECBOOT_MRC_NOT_SUPPORTED      = 46,
  E_SECBOOT_INVALID_EKU_OPTION     = 47, /**< EKU option does not contain Code Signing */
  E_SECBOOT_MAX                    = 0x7FFFFFFF /**< Force to 32 bits */
} secboot_error_type;

 /** @name Secboot Constants
 @{ */
#define SECBOOT_SHARED_API_VERSION 0x2ECB0001UL //0x2ECB00RR - RR:secboot callback release version

#define SECBOOT_CRYPTO_CB_VERSION 0xCA110001UL //0xCA1100RR - RR:secboot callback release version

#define SECBOOT_CM_CB_VERSION 0xC0EA0001UL //0xC0EA00RR - RR:secboot callback release version
 ...
 */
#define SECBOOT_HW_APPS_CODE_SEGMENT   1U /**< Code segment in SECURE_BOOTn register 
                                              representing authentication information 
                                              for the application processors images */     
#define SECBOOT_HW_MBA_CODE_SEGMENT    2U /**< Code segment in SECURE_BOOTn register 
                                              representing authentication information 
                                              for the Modem Boot Authentication (MBA) image */     
#define SECBOOT_HW_MSS_CODE_SEGMENT    3U /**< Code segment in SECURE_BOOTn register 
                                              representing authentication information 
                                              for the modem image */
 ...
 */
#define SECBOOT_HW_SP_BOOT_CODE_SEGMENT 0x80U /**< Code segment in SECURE_BOOTn register 
                                                  representing authentication information 
                                                  for the secure processor image */     
#define SECBOOT_APPS_SEC_CODE_SEGMENT 0x40U /**< Code Segment in Secure Boot for
                                                Secure World code */
#define SECBOOT_HW_QSEE_ROOT_CODE_SEGMENT 0x43U /**< Code Segment in Secure Boot for
                                                    Image using qsee production root for signing */
#define SECBOOT_HW_QSEE_TEST_ROOT_CODE_SEGMENT 0x44U /**< Code Segment in Secure Boot for
                                                         Image using qsee test root for signing */
#define SECBOOT_DEFAULT_CODE_SEGMENT 0x49U /**< Default Code Segment in Secure Boot */
 ...
 */
#define SECBOOT_CTRL_VALIDATE_METADATA          (0xB0A01UL) /**< add and verify metadata
 ...
 * The caller provides secboot context, command type and parameters.
 */
#define SECBOOT_CTRL_AUTH_RESET_FUSE_INFO       (0xB0F00UL) /**< reset/clearup provided fuse info within context,
                                                               - param1 and param2 are set to 0 */
#define SECBOOT_CTRL_AUTH_SET_MSM_HW_ID         (0xB0F01UL) /**< set fuse info: msm hw id,
                                                               - param1 is msm_hw_id and param2 is set to 0 */
#define SECBOOT_CTRL_AUTH_SET_USE_SERIAL_NUM    (0xB0F02UL) /**< set fuse info: use_serial_num
                                                               - param1 is use_serial_num and param2 is set to 0 */
#define SECBOOT_CTRL_AUTH_SET_USE_OEM_ID        (0xB0F03UL) /**< set fuse info: use_oem_id
                                                               - param1 is use_oem_id and param2 is set to 0 */
#define SECBOOT_CTRL_AUTH_SET_SERIAL_NUM        (0xB0F04UL) /**< set fuse info: serial number
                                                               - param1 is serial number and param2 is set to 0 */
#define SECBOOT_CTRL_AUTH_SET_SOC_HW_VERS       (0xB0F05UL) /**< set fuse info: soc hw version
                                                               - param1 is soc hw version and param2 is set to 0 */
#define SECBOOT_CTRL_AUTH_SET_ROOT_OF_TRUST     (0xB0F06UL) /**< set fuse info: root of trust
...
#define SECBOOT_CTRL_AUTH_SET_MRC_FUSE_INFO     (0xB0F07UL) /**< set fuse info: MRC fuse info
                                                               - param1 is pointer to secboot_mrc_fuse_info_type, param2
                                                               is sizeof(secboot_mrc_fuse_info_type)*/
#define SECBOOT_CTRL_AUTH_SET_USE_SUPPLIED_FUSE_INFO (0xB0F08UL) /**< set fuse info: use caller provided fuse info
                                                               - param1 is use_supplied_fuse_info flag and param2 is set to 0 */
#define SECBOOT_CTRL_AUTH_SET_USE_ONLY_SUPPLIED_ROT  (0xB0F09UL) /**< set fuse info: only use root of trust 
                                                               - param1 is use_root_of_trust_only flag and param2 is set to 0 */
#define SECBOOT_CTRL_AUTH_DISABLE_OEM_ID_INDEPENDENT (0xB0F0AUL) /**< set fuse info: disable oem_id_independent 
                                                               - param1 is oem_id_indp_disabled flag and param2 is set to 0 */
#define SECBOOT_CTRL_AUTH_SET_METADATA               (0xB0F0BUL) /**< set auth info: metadata is required to verify the cert chain 
                                                               - param1 is the address to metadata buffer and param2 is buffer size */
#define SECBOOT_CTRL_AUTH_SET_RSA_DISABLE            (0xB0F0CUL) /**< set fuse info: rsa_disable (fuse applies only on OEM signing) 
                                                               - param1 is rsa_disable flag and param2 is set to 0 */
#define SECBOOT_CTRL_AUTH_SET_OEM_ID                 (0xB0F0DUL) /* set fuse info: oem_id
                                                               - param1 is oem_id flag and param2 is set to 0 */
#define SECBOOT_CTRL_AUTH_SET_MODEL_ID               (0xB0F0EUL) /* set fuse info: model_id
 ...
 */
#define SECBOOT_CTRL_AUTH_GET_CRYPTO_FTBL            (0xB0F51UL) /* get crypto ftbl used by the secboot handle (return default, 
 ...
 * Some commands may not need secboot context.
 */
#define SECBOOT_CTRL_HW_API_GET_IS_AUTH_ENABLED   (0xB0FA1UL) /**< get fuse info: auth_en fuse. 
                                                                 - secboot context is required
                                                                 - param1 is pointer to uint32_t and param2 is code_segment */
#define SECBOOT_CTRL_HW_API_GET_ROOT_OF_TRUST     (0xB0FA2UL) /**< get fuse info: OTP root of trust.
                                                                 - secboot context is NOT required
																 - param1 is secboot_hw_api_rot_data buffer and param2 is buffer size */
#define SECBOOT_CTRL_HW_API_GET_MSM_HW_ID         (0xB0FA3UL) /**< get fuse info: msm hw id.
                                                                 - secboot context is NOT required
																 - param1 is secboot_hw_api_msm_hw_id_data buffer and param2 is buffer size */
#define SECBOOT_CTRL_HW_API_GET_USE_SERIAL_NUM    (0xB0FA4UL) /**< get fuse info: use_serial_num.
                                                                 - secboot context is NOT required
                                                                 - param1 is pointer to uint32_t and param2 is code_segment */
#define SECBOOT_CTRL_HW_API_GET_SERIAL_NUM_32        (0xB0FA5UL) /**< get fuse info: serial num
                                                                 - secboot context is NOT required
                                                                 - param1 is pointer to uint32_t and param2 is 0 */
#define SECBOOT_CTRL_HW_API_IS_LEGACY_SECBOOT_ENABLED  (0xB0FA6UL) /**< get fuse info to decide if legacy_secboot (pre-MBNv6)  
                                                                  feature is supported (like hmac). Secboot skips the MBNv6
                                                                  metadata check when this returns 1 (enabled: 1, disabled: 0).
                                                                 - secboot context is NOT required
                                                                 - param1 is pointer to uint32_t and param2 is 0 */
#define SECBOOT_CTRL_HW_API_POPULATE_HW_FUSE_INFO  (0xB0FA7UL) /**< populate hw fuse info to secboot handle */ 
 ...
 */
#define SECBOOT_DEBUG_NOP     0x0U /**< Bit value 00 - No operation needs to be performed */
#define SECBOOT_DEBUG_DISABLE 0x1U /**< Bit value 01-  Write 0 to one-time override registers */                                     
#define SECBOOT_DEBUG_ENABLE  0x2U /**< Bit value 10 - Write 1 to one-time override registers*/
 ...
 *        in the OU field must match the fuse. Absense of the field means disabled.
  */
#define SECBOOT_CRASH_DUMP_DISABLE 0x0U /**< Bit value 0 - crash dump is turned off */                                     
#define SECBOOT_CRASH_DUMP_ENABLE  0x1U /**< Bit value 1 - crash dump is turned on */
 ...
 */
#define SECBOOT_ROOT_KEY_CONTROL_ENABLE   0x0U /**< Bit value 0 - enable write */                                     
#define SECBOOT_ROOT_KEY_CONTROL_DISABLE  0x1U /**< Bit value 1 - disable write */
 ...
 *
 */
#define SECBOOT_ROOT_CERT_SEL_ENABLED  1U /**< Root Cert Selection is enabled */
#define SECBOOT_ROOT_CERT_SEL_DISABLED 0U /**< Root Cert Selection  is disabled */
 ...
 */
typedef struct secboot_handle_t
{
    uint32_t handle[SECBOOT_CONTEXT_HANDLE_SIZE]; /**< 4 byte aligned buffer needed by secboot for it's operations */
}secboot_handle_type;
 ...
 */
typedef struct secboot_image_info_t
{
  const uint8_t* header_ptr_1;   /**< Pointer to the header */
  uint32_t       header_len_1;   /**< Length in bytes of the image header */
  const uint8_t* code_ptr_1;     /**< Pointer to the code */
  uint32_t       code_len_1;     /**< Length in bytes of the image */
  const uint8_t* metadata_info_ptr; /**< Metadata information pointer */
  uint32_t       metadata_len;   /**< Metadata len */
  const uint8_t* x509_chain_ptr; /**< Pointer to the certificate chain */
  uint32_t       x509_chain_len; /**< Length in bytes of the certificate chain */
  const uint8_t* signature_ptr;  /**< Pointer to the signature */
  uint32_t       signature_len;  /**< Length in bytes of the  signature */
  uint32_t       sw_type;        /**< Type of the image being authenticated */
  uint32_t       secondary_sw_id; /**< Secondary SW ID for an image */
  uint32_t       sw_version;     /**< Minimum version of the image that can be executed (for rollback prevention) */
 ...
 * public API.
 */
typedef struct secboot_mrc_fuse_info_t
{
  uint32_t   is_root_sel_enabled;  /**< Is Root Cert Selection enabled, set value as
                                      SECBOOT_ROOT_CERT_SEL_ENABLED/SECBOOT_ROOT_CERT_SEL_DISABLED */
  uint32_t   num_root_certs;       /**< valid only if root cert selection is enabled */
  uint32_t   revocation_list; /**< list of revoked root cert indicies */
  uint32_t   activation_list; /**< list of active root cert indicies */
 ...
 */
typedef struct secboot_hw_api_rot_data_t
{
  uint32_t   code_segment;  /**< code segment, required by hw api*/
  uint8_t    root_of_trust[SECBOOT_OTP_ROOT_OF_TRUST_BYTE_SIZE];   /**< array for root of trust data */
 ...
 */
typedef struct secboot_hw_api_msm_hw_id_data_t
{
  uint32_t   code_segment;  /**< code segment, required by hw api*/
  uint64_t   msm_hw_id;     /**< fuse value integer */
 ...
 */
typedef PACKED_STRUCT secboot_metadata_t
{
...
{
  unsigned long   code_address;       /**< Address (pointer value) of the code that was hashed */
  uint32_t   code_length;        /**< The code length */
  uint32_t   image_hash_length;  /**< Hash length */
  uint8_t    image_hash[SECBOOT_HASH_DIGEST_BYTE_SIZE]; /**< hash of HEADER + CODE */
}secboot_image_hash_info_type;

/**
 * Secboot structure to hold the timestamps for PBL profile markers.
 */
typedef struct secboot_pbl_profile_marker
{
	uint32_t secboot_pbl_authenticate_init_entry;  /**< Start of secboot_authenticate entry */
	uint32_t secboot_pbl_parse_cert_buffer;        /**< Start of parse_cert_buffer */
	uint32_t secboot_pbl_hash_certificates;        /**< Start of hash certificates */
	uint32_t secboot_pbl_verify_cert_chain;        /**< Start of verify cert chain */
	uint32_t secboot_pbl_verify_image_signature;   /**< start of verify image signature */
  uint32_t secboot_pbl_authenticate_close;       /**< secboot_authenticate is complete */
 ...
 */
typedef struct secboot_verified_info_t
{
  uint32_t                       version_id;   /**< The version id (define the secboot lib version) */
  uint64_t                       sw_id;        /**< The software id (upper 32 bits:version, lower 32 bits:type)
											 the image was signed with */
  uint32_t                       enable_debug; /**< Value of the debug settings from the attestation cert, i.e., 
  ...
  uint32_t                       enable_root_key_control; /**< Value to indicate if the configu fuse region is writeable
 ...
 */
typedef struct secboot_api_verify_meta_data_t
{
  const uint8_t* metadata_info_ptr; /**<Metadata information pointer */
  uint32_t       metadata_len;      /**<Metadata len */
  uint32_t       code_segment;      /**< code segment, required by hw api*/
  uint32_t       sw_id;             /**< Type of the image being authenticated */
  uint32_t       secondary_sw_id;   /**< Secondary SW ID for an image */
  uint32_t       sw_version;        /**< Minimum version of the image that can be executed (for rollback prevention) */
 ...
 */
#define E_SECBOOT_CB_SUCCESS                (0U)   /**< Operation was successful. */
#define E_SECBOOT_CB_FAILURE                (1U)   /**< General failure. */
#define E_SECBOOT_CB_INVALID_PARAM          (2U)   /**< Invalid parameter passed into function. */
#define E_SECBOOT_CB_HASH_INIT_FAIL         (3U)   /**< crypto hash init fail. */
#define E_SECBOOT_CB_HASH_UPDATE_FAIL       (4U)   /**< crypto hash update fail. */
#define E_SECBOOT_CB_HASH_FINAL_FAIL        (5U) /**< crypto hash final fail */
#define E_SECBOOT_CB_HASH_DEINIT_FAIL       (6U)   /**< crypto hash deinit fail. */
#define E_SECBOOT_CB_INVALID_CTX_SIZE_FAIL  (7U)   /**< ctx size is not sufficient. */
#define E_SECBOOT_CB_OVERFLOW_FAIL          (15U)   /**< overflow error. */
#define E_SECBOOT_CB_RSA_MODEXP_FAIL        (16U)   /**< rsa modexp operation error. */

/**
 * crypto_ctx_type is moved to secboot_env.h as environment specific. It could requires
 *                 different context variables for the crypto engine in various environments.
 */
 ...
 *    function signatures are the same as crypto_ftbl_type in seccommon.h for direct
 *    assignment (for backward compatibility, we don't want to change crypto_ftbl_type).
 *    
 */
typedef struct secboot_crypto_ftbl_t {
	uint32_t version;         /**< version id - set to SECBOOT_CRYPTO_CB_VERSION */
    crypto_ctx_type handle; /**< caller passes in handle with run-time buffer */
    uint32_t (*HashInit) (crypto_ctx_type* ctx, uint32_t hash_algo);
    uint32_t (*HashUpdate) (crypto_ctx_type* ctx, const void *data, uint64_t data_len);
    uint32_t (*HashFinal) (crypto_ctx_type* ctx, void* digest_ptr, uint32_t digest_len);
    uint32_t (*HashDeInit) (crypto_ctx_type* ctx);
    uint32_t (*ModExp) (crypto_ctx_type* ctx,      // the handle context
                      uint32_t mod_size,                     // the mod size
                      uint32_t exp_size,                     // the exp size
                      uint8_t* mod,                          // the ptr to mod
                      uint8_t* message,                      // the ptr to message
                      uint8_t* exp,                          // the ptr to exp
                      uint8_t* r);                           // the ptr to return value
 ...
 */
typedef struct secboot_cm_ftbl_t {
  uint32_t version;  /**< the version id - set to SECBOOT_CM_CB_VERSION*/
 ...
 * (Data )
 */
typedef struct secboot_ftbl_t
{
  uint32_t              version;        /** Version id - set to SECBOOT_SHARED_API_VERSION */
  secboot_error_type (*secboot_init)  /** Pointer to secboot_init */
                      (secboot_handle_type*                secboot_handle_ptr,
                       ...
                       struct secboot_ftbl_t*           secboot_ftbl_ptr); // set crypto cb function
 ...
 *                                      are required. ModExp is not needed and it 
 *                                      can be set as NULL)
...
(
  ...
  uint32_t                                    hash_algo,
  const uint8_t*                              data1_in_ptr,
  uint64_t                                    data1_len,
  const uint8_t*                              data2_in_ptr,
  uint64_t                                    data2_len,
  uint8_t*                                    digest_ptr,
  uint32_t                                    digest_len
);
...
/**
  ...
  *
  */
uint32_t secboot_get_max_cert_chain_size(void);
...
/**
  ...
  *                                      buffer size is larger than sizeof(uint32_t), buffer
...
#define secboot_verified_info_get_sw_version(verified_info_ptr) \
          ((verified_info_ptr)->metadata.anti_rollback) \
