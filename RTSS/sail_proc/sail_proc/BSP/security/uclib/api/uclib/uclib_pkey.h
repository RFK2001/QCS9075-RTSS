...
typedef enum {
  ...
  UCLIB_EC_CURVE_BRAINPOOL_P192 = 0x11,
  UCLIB_EC_CURVE_BRAINPOOL_P224,
  UCLIB_EC_CURVE_BRAINPOOL_P256,
  UCLIB_EC_CURVE_BRAINPOOL_P320,
  UCLIB_EC_CURVE_BRAINPOOL_P384,
  UCLIB_EC_CURVE_BRAINPOOL_P512,

  UCLIB_EC_CURVE_25519,

  UCLIB_EC_CURVE_MAX
} UCLIB_EC_CURVE_ID;


/* EDDSA INSTANCE types */
typedef enum {
  UCLIB_PKEY_ED25519 = 0,      /* ED25519 */
  UCLIB_PKEY_ED25519CTX,       /* ED25519ctx */
  UCLIB_PKEY_ED25519PH,          /* ED25519ph */
  

  UCLIB_PKEY_ED25519_INSTANCE_MAX
} UCLIB_PKEY_ED25519_TYPE;
...
typedef enum {
  ...
  UCLIB_PKEY_PARAM_SET_KEYSIZE_BITS,
  ...
  /* Accepts a byte-array and length to set the X25519 public key */
  UCLIB_PKEY_PARAM_SET_X25519_PUBKEY,

  /* Accepts a byte-array and length to return the X25519 public key */
  UCLIB_PKEY_PARAM_GET_X25519_PUBKEY,

  /* Sets whether the user want to shared secret as ECC Ephemeral Keys */
  UCLIB_PKEY_PARAM_SET_EPHEMERAL_ECC_KEY,

  /* Sets whether the user want to shared secret as ECC Static Keys */
  UCLIB_PKEY_PARAM_SET_STATIC_ECC_KEY,

  /*set context for Ed25519PH or Ed25519CTX mode*/
  UCLIB_PKEY_PARAM_SET_EDDSA_CONTEXT,

  /*get context for Ed25519PH or Ed25519CTX mode*/
  UCLIB_PKEY_PARAM_GET_EDDSA_CONTEXT, 
  
  /*Indicates EDDSA Instance type*/
  UCLIB_PKEY_PARAM_EDDSA_INSTANCE,
...
/**
 * @brief  Computes: computes r = b**e mod m.  (b < m)
 *         Here Base(b) is in little-endian format.
 *         Exponent(e) and Modulus(m) are in little-endian format.
 *         Exponent, Modulus and Modulus size are set using uclib_pkey_ctrl() API
 *         PKE-HW engine only supports ModExp feature.
 *         This is a temporary provided API .
 *
 * @param h           [in]  handle to the pkey context
 * @param out         [out] pointer to output buffer
 * @param out_buf_len [in]  output buffer length
 * @param base        [in] pointer to Base buffer
 * @param base_len    [in] Length of Base Buffer
 *
 * @return 0 if successful. negative value otherwise.
 */

int uclib_pkey_modexp(UCLIB_PKEY_HANDLE h,
                      uint8_t *out,
                      size_t out_buf_len,
                      const uint8_t * base,
                      size_t base_len);
