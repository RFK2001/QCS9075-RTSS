...
typedef enum {
  ...
  /*stream cipher with 128 bit key size*/
  UCLIB_STREAM128,
...
typedef enum {
  ...
  UCLIB_CCM,
  UCLIB_GCM,
  UCLIB_GCM_STRM,    /* HW GCM Stream-like Input Mode*/
...
typedef enum {
  UCLIB_CIPHER_PHK = 0,             /** H/w keys are only supported in MPSS image for Cipher operation in order to
...
typedef enum {
  ...
  UCLIB_CIPHER_SET_CMN_POOL_KEY,    /** Sets commpon pool key slots  */
...
int uclib_cipher_init(UCLIB_CIPHER_HANDLE h,
                      ...
                      bool  encrypt) __attribute__((used));
#else
int uclib_cipher_init(UCLIB_CIPHER_HANDLE h,
                      UCLIB_CIPHER_ALG alg,
                      UCLIB_CIPHER_MODE mode,
