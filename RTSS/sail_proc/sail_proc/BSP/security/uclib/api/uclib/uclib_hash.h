...
typedef enum {
  UCLIB_HMAC_SHK = 0,                 /** Secondary HW Key - OEM key */
  UCLIB_HMAC_PHK,                     /** Primary HW Key - QC key    */
...
typedef enum {
  ...
  UCLIB_HASH_SET_WRAPPED_KEY,         /** Accepts data buffer and size. Used to set a wrapped key                            */

  UCLIB_HASH_SET_CMN_POOL_KEY,        /** Sets the common pool and Slot offset  */
...
int uclib_hash_init(UCLIB_MD_HANDLE h, UCLIB_HASH_ALG alg, UCLIB_ENGINE_TYPE engine) __attribute__((used));
#else
